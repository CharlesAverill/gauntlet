...
static bool8 ExtractMonDataToSummaryStruct(struct Pokemon *mon)
{
    ...
    default:
        ...
        sMonSummaryScreen->relearnableMovesNum = P_SUMMARY_SCREEN_MOVE_RELEARNER ? 10 : 0;
        return TRUE;
    }
    sMonSummaryScreen->switchCounter++;
    return FALSE;
...
static void SetPokerusCuredSprite(void)
{
    ...
    gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_POKERUS_CURED]].invisible = TRUE;
}
...
static void ShowCancelOrRenamePrompt(void)
{
    const u8 *promptText = ShouldShowRename() ? (const u8*)"RENAME" : gText_Cancel2;
