...
static struct MonChoiceData sStarterChoices[9]; 
// static bool initialized_starters = FALSE;
// {
...
static void CreateHandSprite()
{
    ...
    // Initialize random starters
    for(i=BALL_TOP_FIRST; i <= BALL_BOTTOM_SECOND; i++) {
        sStarterChoices[i].species = Random() % 152;
        sStarterChoices[i].level = 50;
        sStarterChoices[i].ball = (Random() % ITEM_CHERISH_BALL) + 1;
    }
